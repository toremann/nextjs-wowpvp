...
function Player({ player }) {
    const classes = {
        ...
        8: 'Unknown class',
        9: <text style={{ color: 'purple' }}>Warlock</text>,
        10: 'Unknown class',
    ...
    const winrate2v2 = (Number(player.wins2v2) / (Number(player.wins2v2) + Number(player.loss2v2))) * 100;

    const played3v3 = player.wins3v3 + player.loss3v3;
    const winrate3v3 = (Number(player.wins3v3) / (Number(player.wins3v3) + Number(player.loss3v3))) * 100;

    const playedrbg = player.winsrbg + player.lossrbg;
    const winraterbg = (Number(player.winsrbg) / (Number(player.winsrbg) + Number(player.lossrbg))) * 100;
        ...
        <>
            <div className={styles.card__top}>
                <div className={styles.card__playername}>
                    <h3>
                        {player.player} - {player.realm} <br /> {classes[player.class]} - <b>ilvl:</b> {player.ilvl}
                    </h3>
                    <br />
                </div>
            ...
            <div className={styles.stats}>
                <div className={styles.stats__divs}>
                    ...
                    <p>
                        <b>Rating:</b> {player.currentrating2v2} / {last2v2.rating - secondLast2v2.rating}
                    </p>
                    <p>
                ...
                <div className={styles.stats__divs}>
                    ...
                    <p>
                        <b>Rating:</b> {player.currentrating3v3} / {last3v3.rating - secondLast3v3.rating}
                    </p>
                    <p>
                    ...
                    <p>
                        <b>Ratio:</b> {checkNan3v3 ? <text>0</text> : <text>{winrate3v3.toFixed(0)}%</text>}
                    </p>
                </div>
                <div className={styles.stats__divs}>
                    <p>
                    ...
                    <p>
                        <b>Rating:</b> {player.currentratingrbg} / {lastRbg.rating - secondLastRbg.rating}
                    </p>
                    <p>
                    ...
                    <p>
                        <b>Ratio:</b> {checkNanRBG ? <text>0</text> : <text>{winraterbg.toFixed(0)}%</text>}
                    </p>
                </div>
            </div>
        </>
