...
function Stats({ players, isConnected }) {
    ...
    const totalWinRatio2v2 = (Number(totalWins2v2) / (Number(totalWins2v2) + Number(totalLoss2v2))) * 100;
    ...
    const totalWinRatio3v3 = (Number(totalWins3v3) / (Number(totalWins3v3) + Number(totalLoss3v3))) * 100;
    ...
    const totalWinRatioRbg = (Number(totalWinsRbg) / (Number(totalWinsRbg) + Number(totalLossRbg))) * 100;
    ...
    return (
        <>
            ...
            <div className={styles.stats__total}>
                ...
                <b>Ratio:</b> {checkNan2v2 ? <text>0</text> : <text>{totalWinRatio2v2.toFixed(0)}% </text>}
                <hr />
                <b>TOTAL 3v3:</b> <br />
                <b>Wins:</b> {totalWins3v3} <br />
                <b>Loss:</b> {totalLoss3v3} <br />
                <b>Games:</b> {totalWins3v3 + totalLoss3v3} <br />
                <b>Ratio:</b> {checkNan3v3 ? <text>0</text> : <text>{totalWinRatio3v3.toFixed(0)}% </text>}
                <hr />
                <b>TOTAL RBG:</b> <br />
                <b>Wins:</b> {totalWinsRbg} <br />
                <b>Loss:</b> {totalLossRbg} <br />
                <b>Games:</b> {totalWinsRbg + totalLossRbg} <br />
                <b>Ratio:</b> {checkNanRBG ? <text>0</text> : <text>{totalWinRatioRbg.toFixed(0)}% </text>}
                <hr />
                ...
                <br />
                <b>Database:</b> {isConnected ? <text style={{ color: 'green' }}>All good</text> : <text style={{ color: 'red' }}> Database error</text>}
            </div>
        </>
    );
}
